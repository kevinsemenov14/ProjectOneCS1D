...
void MainWindow::on_normalTrip_clicked()
{
    ...
    QQueue<QString> temp = GetSqlDistances(startingCity);

    for(int count = 0; count < citiesToVisit.size(); count++)
    {
        ...
        temp = GetSqlDistances(sortedQueueToVisit.at(sortedQueueToVisit.size() - 1));
    }
